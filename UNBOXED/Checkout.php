...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['place_order'])) {
    ...
    $sql = "INSERT INTO order_item (customer_id, customer_name, address, contact, product_id, product_img, product_name, product_owner, quantity, price, total_payment, payment, order_date, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Pending')";
// Prepare and execute statement
$stmt = $conn->prepare($sql);

if ($stmt) {
    // Bind parameters
    $stmt->bind_param("isssissssdsss", $customer_id, $customer_name, $address, $contact, $product_id, $product_img, $product_name, $product_owner, $quantity, $price, $total_payment, $payment, $order_date);
