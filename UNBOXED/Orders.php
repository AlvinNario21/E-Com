...
// Retrieve customer ID from session (assuming it's stored there)
if(isset($_SESSION['name'])) {
...
$statusQueries = array(
    "All" => "SELECT COUNT(*) AS count FROM order_item WHERE product_owner = $customer_id",
    "Pending" => "SELECT COUNT(*) AS count FROM order_item WHERE product_owner = $customer_id AND status = 'Pending'",
    "To Ship" => "SELECT COUNT(*) AS count FROM order_item WHERE product_owner = $customer_id AND status = 'To Ship'",
    "To Receive" => "SELECT COUNT(*) AS count FROM order_item WHERE product_owner = $customer_id AND status = 'To Receive'",
    "Completed" => "SELECT COUNT(*) AS count FROM order_item WHERE product_owner = $customer_id AND status = 'Completed'",
    "Rated" => "SELECT COUNT(*) AS count FROM order_item WHERE product_owner = $customer_id AND status = 'Rated'",
);
...
<ul class="nav nav-tabs">
    <li class="nav-item">
        <a class="nav-link" href="?status=All">All <span class="badge bg-primary"><?php echo $statusCounts['All'] ?? 0; ?></span></a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?status=Pending">Pending <span class="badge bg-primary"><?php echo $statusCounts['Pending'] ?? 0; ?></span></a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?status=To Ship">To Ship <span class="badge bg-primary"><?php echo $statusCounts['To Ship'] ?? 0; ?></span></a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?status=To Receive">To Receive <span class="badge bg-primary"><?php echo $statusCounts['To Receive'] ?? 0; ?></span></a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?status=Completed">Completed <span class="badge bg-primary"><?php echo ($statusCounts['Completed'] ?? 0) + ($statusCounts['Rated'] ?? 0); ?></span></a>
    </li>
</ul>

<?php
function constructQuery($selectedStatus, $customer_id, $username) {
    switch ($selectedStatus) {
        case 'All':
            return "SELECT * FROM order_item WHERE customer_id = $customer_id AND product_owner = '$username'";
        case 'Pending':
        case 'To Ship':
        case 'To Receive':
            return "SELECT * FROM order_item WHERE customer_id = $customer_id AND product_owner = '$username' AND status = '$selectedStatus'";
        case 'Completed':
            return "SELECT * FROM order_item WHERE customer_id = $customer_id AND product_owner = '$username' AND (status = 'Completed' OR status = 'Rated')";
        default:
            return ""; // Invalid status, return an empty query
    }
}
...
// Example customer_id and username, replace these with your actual customer ID and username retrieval logic
$customer_id = $_SESSION['customer_id'];
$username = $_SESSION['name'];

// Function to display order details based on selected status and customer_id
function displayOrderDetails($conn, $selectedStatus, $customer_id, $username) {
    // Construct SQL query
    $sql = constructQuery($selectedStatus, $customer_id, $username);
...
displayOrderDetails($conn, $selectedStatus, $customer_id, $username);
?>
...
document.getElementById('searchButton').addEventListener('click', function() {
    ...
    if (searchInput !== '') {
        ...
        window.location.href = 'Orders.php?search=' + encodeURIComponent(searchInput);
    }
});
