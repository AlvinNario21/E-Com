...
$statusQueries = array(
    "All" => "SELECT COUNT(*) AS count FROM order_item WHERE product_owner = $name",
    "Pending" => "SELECT COUNT(*) AS count FROM order_item WHERE product_owner = $name AND status = 'Pending'",
    "To Ship" => "SELECT COUNT(*) AS count FROM order_item WHERE product_owner = $name AND status = 'To Ship'",
    "To Receive" => "SELECT COUNT(*) AS count FROM order_item WHERE product_owner = $name AND status = 'To Receive'",
    "Completed" => "SELECT COUNT(*) AS count FROM order_item WHERE product_owner = $name AND status = 'Completed'",
    "Rated" => "SELECT COUNT(*) AS count FROM order_item WHERE product_owner = $name AND status = 'Rated'",
);
...
<ul class="nav nav-tabs">
    ...
    <li class="nav-item">
        <a class="nav-link" href="?status=Pending">Pending</span></a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?status=To Ship">To Ship</span></a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?status=To Receive">To Receive</span></a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?status=Completed">Completed</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?status=Cancelled">Cancelled</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?status=Rated">Rated</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?status=Returned">Return&Refund</a>
    </li>
</ul>

<?php
function constructQuery($selectedStatus, $username) {
    switch ($selectedStatus) {
        case 'All':
            return "SELECT * FROM order_item WHERE product_owner = '$username' AND status != 'Returned'";
        case 'Pending':
        case 'To Ship':
        case 'To Receive':
        case 'Cancelled':
            return "SELECT * FROM order_item WHERE product_owner = '$username' AND status = '$selectedStatus'";
        case 'Completed':
            return "SELECT * FROM order_item WHERE product_owner = '$username' AND status = 'Completed'";
        case 'Rated':
            return "SELECT * FROM order_item WHERE product_owner = '$username' AND status = 'Rated'";
        case 'Returned':
            return "SELECT * FROM order_item WHERE product_owner = '$username' AND status = 'Returned'";
        default:
...
function displayOrderDetails($conn, $selectedStatus, $username) {
    // Construct SQL query
    $sql = constructQuery($selectedStatus, $username);
    ...
    if ($result) {
        ...
        if ($result->num_rows > 0) {
            ...
            foreach ($orderItems as $row) {
                ...
                // Check if status is "Returned" to disable the checkbox
                $disabled = ($row['status'] == 'Returned') ? 'disabled' : '';
            
                echo "<input type='checkbox' class='item-checkbox' name='checked_items[]' value='" . $row['order_item_id'] . "' data-product-id='" . $row['product_id'] . "' data-customer-name='" . $row['customer_name'] . "' $disabled>"; // Checkbox for each item
...
displayOrderDetails($conn, $selectedStatus, $username);
?>
...
<div class="modal fade" id="shipmentModal" tabindex="-1" aria-labelledby="shipmentModalLabel" aria-hidden="true">
    ...
    <input type="text" class="form-control-ship" id="recipient-name" name="recipient-name" readonly>
                    ...
                    <div class="mb-3">
                        <label for="product-id" class="form-label-prod">Product ID:</label>
                        <input type="text" class="form-control-ship" id="product-id" name="product-id" readonly>
                    </div>
