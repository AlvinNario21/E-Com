...
<nav class="navbar navbar-expand-lg bg-body-tertiary">
    ...
    <?php
// Assuming the parameter name for owner in the URL is 'owner'
$owner = isset($_GET['owner']) ? $_GET['owner'] : '';

if (!empty($owner)):
?>
    <a class="navbar-brand">|</a>
    <a class="navbar-brand"><?php echo htmlspecialchars($owner); ?></a> <!-- Display showcase_name here -->
<?php endif; ?>
...
// Check if the user is logged in and the necessary parameters are set
if (!empty($name) && isset($_GET['showcase_id']) && isset($_GET['showcase_name'])) {
    ...
    if ($check_owner_result && $check_owner_result->num_rows > 0) {
        ...
        // Check if the name from session matches the owner from the showcase
...
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
            ...
            echo '        <a href="Product.php?product_id=' . $row['product_id'] . '&product_img=' . urlencode($row['product_img']) . '&product_name=' . urlencode($row['name']) . '&product_price=' . urlencode($row['price']) . '&product_description=' . urlencode($row['description']) . '&showcase_id=' . $showcase_id . '&showcase_name=' . urlencode($showcase_name) . '&owner=' . urlencode($owner) .'">';
