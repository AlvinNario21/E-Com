...
<div class="product-card-container">
    ...
    if(isset($_GET['name']) && !empty($_GET['name']) && isset($_GET['image_dp']) && !empty($_GET['image_dp'])) {
        ...
        $sql = "SELECT s.showcase_id, s.showcase_name, p.product_id, p.product_img, p.name, p.price, p.description 
        FROM showcase s 
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                // Wrap each product card with an anchor tag linking to Product.php
